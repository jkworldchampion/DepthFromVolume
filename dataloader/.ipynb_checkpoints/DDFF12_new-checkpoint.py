...
class DDFF12Loader(Dataset):
    ...
    def __init__(self, hdf5_filename,  stack_key="stack_train", disp_key="disp_train", transform=None,
        ...
        assert n_stack <= self.max_n_stack, 'DDFF12 has maximum 10 images per stack!'  # stack이 10 넘으면 안됌
    ...
    # focal_stack -> (10, 383, 552, 3)
    # disparity_map -> (383, 552)
    def __getitem__(self, idx):
        # Create sample dict
        try:
            if 'test' in self.stack_key:
                sample =  {'input': self.hdf5[self.stack_key][idx].astype(float),  # (10,383,552,3)
                           'output': np.ones([2,2])}  # empty
            else:
                sample = {'input': self.hdf5[self.stack_key][idx].astype(float),  # (10,383,552,3)
                          'output': self.hdf5[self.disp_key][idx]}   # (383,552)
        except:
            sample = None
            for _ in range(100):
        ...
        # val이고, b_test가 false일 경우 [384, 576] -> [256, 256]  # default를 위해 256으로
    ...
    # GT disparity가 일정 범위 밖이면 0으로 masking
